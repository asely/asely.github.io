...
function pxp_reload(sShare, sPath){
	if(sShare == pxp_sShare  &&  sPath == pxp_sPath)
		window.frames[pxp_sShare + '_reload_' + pxp_sCallId].document.location.href = pxp_sURL + "/action.php?sShare=" + pxp_sShare + "&sPath=" + encodeURIComponent(sPath) + "&sAction=directory_detailed&bReload=true"
}
...
function pxp_reloadAdd(id, objFile){
  ...
  function createCell(width, align){
	...
	ndDivLine.className = "lineDiv"
		...
		var ndDivPermissions = createCell((parseInt(document.frmAction.columnWidth_permissions.value) + 8), 'right')
		ndDivPermissions.innerHTML = '&nbsp;' + objFile.permissions + '&nbsp;'

		var ndDivOwner = createCell((parseInt(document.frmAction.columnWidth_owner.value) + 8), 'left')
		ndDivOwner.innerHTML = '&nbsp;' + objFile.owner + '&nbsp;'

		var ndDivGroup = createCell((parseInt(document.frmAction.columnWidth_group.value) + 8), 'left')
		ndDivGroup.innerHTML = '&nbsp;' + objFile.group + '&nbsp;'
	...
	ndDivLine.appendChild(ndDivPermissions)
	ndDivLine.appendChild(ndDivOwner)
	ndDivLine.appendChild(ndDivGroup)
...
function pxp_reloadUpdate(id, objFile){
	...
	ndFile.childNodes[6].innerHTML = "&nbsp;" + objFile.permissions + "&nbsp;"
	ndFile.childNodes[7].innerHTML = "&nbsp;" + objFile.owner + "&nbsp;"
	ndFile.childNodes[8].innerHTML = "&nbsp;" + objFile.group + "&nbsp;"
